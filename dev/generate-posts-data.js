...
import { blogDir, getSrcDirectoryPath, vaquejadaDir } from "./getSrcDirectoryPath.js";
...
function writePostDataToJson(postData, filepath) {
  console.log('### writePostDataToJson', postData, filepath);
...
export async function generatePostsData() {
  ...
  const guidePosts = await getPostInfoFromDir(vaquejadaDir);
  writePostDataToJson(guidePosts, `${srcDirectory}\/posts\/vaquejada-post-data.json`);
}
